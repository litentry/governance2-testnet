...
```bash
./target/release/governance2 --dev
```

Purge the development chain's state:

```bash
./target/release/governance2 purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/governance2 -ldebug --dev
```
...
$ ./target/release/governance2 --dev --base-path ./my-chain-state/
...
```shell
./target/release/governance2 --help
```
...
(`cargo build --release && ./target/release/governance2 --dev --ws-external`)
by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/governance2 --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/governance2 purge-chain --dev
